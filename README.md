...
`isolation`, `timeout`, `agent`, `checks` and the three `watchdog*` keys are resolved **per project** (`resolveProjectConfig`): CLI flags > that repo's `.codesema/config.json` > `~/.config/codesema/config.json`. Launching the workspace from repo A no longer copies A's isolation, timeout or agent onto repo B. A task's `isolation` is decided at creation and then immutable: changing the project's config never rewrites a record that already exists. The load cap stays **global-only by construction** — it caps the machine, not the repository. Sync fields stay global-only too.
