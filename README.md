...
`isolation`, `isolationAllowedDomains`, `timeout`, `agent` and the three `watchdog*` keys are resolved **per project** (`resolveProjectConfig`): CLI flags > that repo's `.codesema/config.json` > `~/.config/codesema/config.json`. `checks` is repo-only and re-read on every run (`readChecksConfig`). Launching the workspace from repo A no longer copies A's isolation, timeout, allowlist or agent onto repo B. A task's `isolation` is decided at creation and then immutable: changing the project's config never rewrites a record that already exists. Three keys stay **global-only by construction**, because what they bound is the machine or the whole workspace rather than one repository: `maxConcurrentAgents`, its deprecated alias `maxParallelTasks`, and `taskRetentionCount`. A repo file that sets any of them is ignored **and named at boot** — never dropped in silence. Sync fields stay global-only too, silently (they are credentials).
