...
import {
  ...
  loadTask,
  onStoreUnreadable,
  peekHubTicketIdCache,
  readTaskChecks,
  readTaskEvents,
  readTaskJournal,
  removeTaskDir,
  resetHubTicketIdCache,
  resetJournalCursors,
  resetStoreReports,
  saveTask,
  setJournalReader,
...
describe('removeTaskDir', () => {
  ...
  // Arm/hub integration: `hubTicketIdCache` is WRITE-ONCE for a LIVE
  ...
  test('evicts the removed task from hubTicketIdCache', () => {
    const task = createTask(cwd, { ...input, hubTicket: { id: 'tkt-1', title: 't' } })
    expect(peekHubTicketIdCache(cwd, task.id)).toBe('tkt-1')

    expect(removeTaskDir(cwd, task.id)).toBe(true)

    expect(peekHubTicketIdCache(cwd, task.id)).toBeUndefined()
  })
