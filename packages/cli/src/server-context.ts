...
export type ServerConvention = { id: string; rule: string; category: string | null; scope: string | null }
...
function parseConvention(raw: unknown): ServerConvention | null {
  ...
  if (!isString(r.id) || !isString(r.rule) || !isNullableString(r.category) || !isNullableString(r.scope)) return null
...
function parseLearnedRule(raw: unknown): ServerLearnedRule | null {
  if (!raw || typeof raw !== 'object') return null
  const r = raw as Record<string, unknown>
  if (!isString(r.id) || !isString(r.rule)) return null
...
export function parseServerContextPayload(raw: unknown): ServerContextPayload | null {
  ...
  if (!r.repo || typeof r.repo !== 'object') return null
  const repo = r.repo as Record<string, unknown>
  if (!isNullableString(repo.remote_url)) return null

  let freshness: ServerContextPayload['freshness'] = null
  if (r.freshness !== null && r.freshness !== undefined) {
    if (typeof r.freshness !== 'object') return null
    const f = r.freshness as Record<string, unknown>
    if (!isNullableString(f.scan_sha) || !isNullableString(f.scanned_at)) return null
  ...
  if (conventions === null || learnedRules === null || facts === null) return null
...
function staleWarning(payload: ServerContextPayload, cwd: string): string | null {
  ...
  if (!sha) return null
  if (isAncestor(sha, 'HEAD', cwd)) return null
