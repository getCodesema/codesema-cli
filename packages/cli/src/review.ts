...
export async function runSimpleFlow(opts: {
  ...
  try {
    ...
    return { ok: false, failure: 'output', message: err instanceof Error ? err.message : String(err), rawOutput: out }
  }
}
...
export async function review(opts: {
  ...
  const { url, stop } = await startServer(session, {
    ...
    mrReviewRunner: createMrReviewRunner({ cwd: input.repo_root, session, agentCommand, timeoutMs }),
  })
