// Turns a hub ticket (a ticket the local hub owns and this arm claimed)
// into a queued task: the symmetric twin of task-issue.ts's
// resolveIssueOrigin/admitIssue, but for a ticket the hub already resolved
...
// (sanitizeArmTicket, contract/arm.ts) by whoever claimed it from the
// hub, so admission is a pure, synchronous lint: lintTicketBody (T2.3),
...
// and prompt (task-server.ts folds `resolveHubTicketOrigin`'s `criteria`
...
// hub-ticket task's very first turn already reads `taskCriteria(record)`
...
// validation nobody is coming to give: the hub validated them already.
...
/**
 * What `resolveHubTicketOrigin` hands back: the same shape task-server.ts's
 * own (unexported) `TaskOrigin` accepts on its `ok: true` branch: title,
 * prompt, no forge issue (a hub ticket is not reconciled against a live
 * forge issue the way T2.4's own origin is; `hubTicket.url` is a plain
 * pointer, not a reconciliation anchor), and the two hub-only fields
 * (`hubTicket`, `criteria`) `task-server.ts`'s `create()` folds onto the
...
/**
 * Validates a hub ticket and derives the task it would become. `cwd` is
 ...
 * the body: a ticket the hub itself would not have been able to publish
 ...
 */
export function resolveHubTicketOrigin(_cwd: string, ticket: ArmTicket): HubTicketOrigin {
  ...
  return {
    ...
    issueSnapshot: null,
    coverageGap: false,
    hubTicket: { id: ticket.id, title, ...(url ? { url } : {}) },
    criteria: lint.body.acceptance_criteria,
  }
}

/**
 * Creates a task from a hub ticket. Resolves `cwd` to its registered
...
export async function createHubTicketTask(
  ...
  return manager.create(project.id, { hubTicket: ticket, autoShip: true })
}
