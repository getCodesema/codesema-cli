...
import { buildServerContext, parseServerContextPayload, type ServerContextPayload } from './server-context.js'
...
describe('parseServerContextPayload', () => {
  ...
  const invalidPayloads: [string, unknown][] = [
    ...
    ['a convention missing rule', { ...VALID_PAYLOAD, conventions: [{ id: 'c1', category: null, scope: null }] }],
    ['a learned_rule missing id', { ...VALID_PAYLOAD, learned_rules: [{ rule: 'x' }] }],
    ['facts holding a non-string', { ...VALID_PAYLOAD, facts: ['ok', 42] }],
    ['freshness with a non-string scan_sha', { ...VALID_PAYLOAD, freshness: { scan_sha: 1, scanned_at: null } }],
  ]
...
describe('buildServerContext', () => {
  ...
  function seedCredentials(url: string): void {
    saveGlobalConfig({ ...loadGlobalConfig(), syncUrl: url, syncWorkspaceId: 'ws-1', syncSecret: 'secret-1' })
  }
  ...
  test('freshness ancestor of HEAD: no staleness warning', async () => {
    ...
    const payload: ServerContextPayload = { ...VALID_PAYLOAD, freshness: { scan_sha: sha, scanned_at: '2026-08-01T00:00:00.000Z' } }
  ...
  test('403 on an unlinked workspace: degrades to null, never throws', async () => {
    stub = await startStubServer((_req, res) => sendJson(res, 403, { error: 'workspace not linked' }))
    seedCredentials(stub.url)
  ...
  test('stale sha (not an ancestor of HEAD): prefixes an explicit warning naming scanned_at', async () => {
    const payload: ServerContextPayload = {
      ...VALID_PAYLOAD,
      freshness: { scan_sha: 'deadbeefdeadbeefdeadbeefdeadbeefdeadbeef', scanned_at: '2026-07-30T09:00:00.000Z' },
    }
    stub = await startStubServer((_req, res) => sendJson(res, 200, payload))
    seedCredentials(stub.url)
