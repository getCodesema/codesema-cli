...
`,
  ...
  // --- task ship ---
  // T3.5: the description is now the recap's own markdown rendering. These two
  // are what stands in its place when there is no recap to render — an empty
  // body would read as "nothing happened", which is the one thing it must not
  // say. ('ship.mrReviewLine' is gone with the composition it belonged to: the
  // review verdict line is no longer part of the description.)
  'ship.mrNoRecap':
    'No recap could be produced for this branch: see the conversation in codesema for what was done.',
  'ship.mrRecapBlocked':
    'The recap of this branch was held back: it looks like it carries a secret, so nothing was sent to the forge. It is on the machine that produced it, in codesema.',
  // Kept apart from the line above on purpose: "a secret was found" and
  // "nobody could look" are different facts, and saying the first when only
  // the second happened sends a reader hunting for a leak nothing reported.
  'ship.mrRecapUnscanned':
    'The recap of this branch was held back: the check that clears it of secrets could not run, so nothing was sent to the forge. It is on the machine that produced it, in codesema.',
...
`,
  ...
  // --- task ship ---
  'ship.mrNoRecap':
    'Aucun récapitulatif n’a pu être produit pour cette branche : voir la conversation dans codesema pour ce qui a été fait.',
  'ship.mrRecapBlocked':
    'Le récapitulatif de cette branche a été retenu : il semble porter un secret, rien n’a donc été envoyé à la forge. Il reste sur la machine qui l’a produit, dans codesema.',
  'ship.mrRecapUnscanned':
    'Le récapitulatif de cette branche a été retenu : le contrôle qui le déclare exempt de secret n’a pas pu s’exécuter, rien n’a donc été envoyé à la forge. Il reste sur la machine qui l’a produit, dans codesema.',
