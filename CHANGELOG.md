...
- "Run review" and "Run dual review" buttons on the MR detail panel: fetches the MR's source branch, reviews it (simple or dual) in a disposable `git worktree` under the OS temp dir, archives the result in the main repo's `.codesema/reviews`, and always removes the worktree afterwards, success or failure. The run streams into the same live web UI as `codesema review` (status, partials, judge decisions). Only one MR review can run at a time; the sidebar marks which MR is currently under review. New endpoints `POST /api/mrs/review` (body `{ number, mode }`) and `GET /api/mrs/review/status`, protected by the same per-server CSRF token pattern as `POST /api/fix`.
